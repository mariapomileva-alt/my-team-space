-- Account-level Lemon Squeezy plans (Single Team vs Academy) + team limits.

-- ---------------------------------------------------------------------------
-- coach_subscriptions (one row per coach account — billing source of truth)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if sub.plan_type = 'single_team' and sub.primary_team_id is not null and sub.primary_team_id <> p_team_id then
    return false;
  end if;
  ...
  if sub.plan_type = 'single_team' then
    if primary_id is null then
      ...
      if primary_id is not null then
        ...
        set primary_team_id = primary_id, updated_at = now()
...
begin
  ...
  update public.coach_subscriptions
  set primary_team_id = p_team_id, updated_at = now()
  where user_id = uid;
...
-- ---------------------------------------------------------------------------
-- create_team: enforce team limit
-- ---------------------------------------------------------------------------
...
begin
  ...
  if owned = 0 then
    update public.coach_subscriptions
    set primary_team_id = tid, updated_at = now()
    where user_id = uid and primary_team_id is null;
    update public.teams set is_plan_primary = true where id = tid;
  end if;
...
-- ---------------------------------------------------------------------------
-- Upsert coach subscription from webhook (service role)
-- ---------------------------------------------------------------------------
