...
export function GoogleAuthButton({ nextPath = "/admin", onError, label = "Continue with Google" }: Props) {
  ...
  async function onClick() {
    ...
    const redirectTo = authCallbackUrl(getAppOrigin(), nextPath);
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={pending}
      className="flex w-full items-center justify-center gap-2.5 rounded-full border border-zinc-200 bg-white py-2.5 text-sm font-semibold text-zinc-800 shadow-sm transition hover:bg-zinc-50 disabled:opacity-60"
    >
      <GoogleMark />
